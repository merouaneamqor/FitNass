...
import { fetchGymsByLocation } from '@/actions/gymActions'; // Import the new action
...
export default async function CityGymsPage({ params }: CityGymsPageProps) {
  ...
  try {
    gyms = await fetchGymsByLocation(city);
  } catch (e) {
    console.error("Failed to fetch gyms:", e);
    error = `Could not load gyms for ${city}. Please try again later.`;
    // Optionally, you could return a more specific error page or component here
  }
